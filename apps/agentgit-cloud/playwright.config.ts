...
const webServerEnv = `AUTH_SECRET=${e2eSecret} AUTH_URL=http://${host}:${port} NEXTAUTH_URL=http://${host}:${port} AUTH_TRUST_HOST=true AUTH_ENABLE_DEV_CREDENTIALS=true AUTH_ALLOW_DEV_CREDENTIALS_IN_PRODUCTION=false AUTH_GITHUB_ID=${e2eGithubId} AUTH_GITHUB_SECRET=${e2eGithubSecret} AUTH_WORKSPACE_ID=ws_e2e_01 AUTH_WORKSPACE_NAME='E2E Workspace' AUTH_WORKSPACE_SLUG=e2e-workspace AGENTGIT_ROOT=${JSON.stringify(
  e2eRoot,
