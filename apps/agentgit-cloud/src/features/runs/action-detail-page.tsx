    ...
    <>
      ...
      <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
        ...
        <Card>
          ...
          <div className="mt-2">
            <Badge tone={decisionTone(action.policyOutcome.decision)}>{action.policyOutcome.decision ?? "unknown"}</Badge>
          </div>
        ...
        <Card className="space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-2 rounded-[var(--ag-radius-md)] border border-[var(--ag-border-subtle)] bg-[var(--ag-bg-elevated)] p-4">
              ...
              <div className="text-sm text-[var(--ag-text-secondary)]">Surface: {action.normalizedAction.executionSurface}</div>
              <div className="text-sm text-[var(--ag-text-secondary)]">Mode: {action.normalizedAction.executionMode}</div>
              <div className="text-sm text-[var(--ag-text-secondary)]">Reversibility: {action.normalizedAction.reversibilityHint}</div>
            </div>
            <div className="space-y-2 rounded-[var(--ag-radius-md)] border border-[var(--ag-border-subtle)] bg-[var(--ag-bg-elevated)] p-4">
              <div className="text-xs uppercase tracking-[0.12em] text-[var(--ag-text-tertiary)]">Timing</div>
              <div className="text-sm text-[var(--ag-text-secondary)]">Occurred {formatRelativeTimestamp(action.occurredAt)}</div>
              <div className="text-sm text-[var(--ag-text-secondary)]">{formatAbsoluteDate(action.occurredAt)}</div>
              <div className="text-sm text-[var(--ag-text-secondary)]">Snapshot: {action.execution.snapshotId ?? "none"}</div>
              <div className="text-sm text-[var(--ag-text-secondary)]">Step ID: {action.execution.stepId ?? "n/a"}</div>
            </div>
          </div>

          <div className="space-y-3">
            <h3 className="text-sm font-semibold uppercase tracking-[0.12em] text-[var(--ag-text-tertiary)]">Policy reasoning</h3>
            <div className="grid gap-3 md:grid-cols-2">
              <Card className="space-y-2">
                ...
                <div className="text-sm text-[var(--ag-text-secondary)]">
                  {action.policyOutcome.approvalRequired ? "Approval gate was required." : "No approval gate was required."}
                </div>
              </Card>
              <Card className="space-y-2">
              ...
              {action.policyOutcome.reasons.length > 0 ? (
                <div className="space-y-2">
                  {action.policyOutcome.reasons.map((reason) => (
                    ...
                    >
                      <div className="text-xs uppercase tracking-[0.12em] text-[var(--ag-text-tertiary)]">{reason.severity}</div>
        ...
        <div className="space-y-6">
          <Card className="space-y-3">
            ...
            <div className="space-y-2 text-sm">
              ...
              <div>
                Run status: <Badge tone={action.runContext.status === "failed" ? "error" : action.runContext.status === "completed" ? "success" : "warning"}>{action.runContext.status}</Badge>
              </div>
              <div>
                Started: <span className="font-medium">{formatAbsoluteDate(action.runContext.startedAt)}</span>
              </div>
              <div>
                Latest event: <span className="font-medium">{formatRelativeTimestamp(action.runContext.latestEventAt)}</span>
              </div>
              <div>
          ...
          <Card className="space-y-3">
            ...
            {action.approvalContext ? (
              <div className="space-y-2 text-sm">
                <div>
                  Status: <Badge tone={approvalTone(action.approvalContext.status)}>{action.approvalContext.status}</Badge>
                </div>
                <div>
                  Requested: <span className="font-medium">{formatAbsoluteDate(action.approvalContext.requestedAt)}</span>
                </div>
                <div>
                  Decision path: <span className="font-medium">{action.approvalContext.decisionRequested}</span>
                </div>
                <div>
                  Resolution note: <span className="font-medium">{action.approvalContext.resolutionNote ?? "none"}</span>
                </div>
                <div>
                ...
                {action.approvalContext.primaryReason ? (
                  <div className="rounded-[var(--ag-radius-sm)] border border-[var(--ag-border-subtle)] px-3 py-2">
                    <div className="text-xs uppercase tracking-[0.12em] text-[var(--ag-text-tertiary)]">Primary reason</div>
                    <div className="text-sm font-medium">{action.approvalContext.primaryReason.code}</div>
                    <div className="text-sm text-[var(--ag-text-secondary)]">{action.approvalContext.primaryReason.message}</div>
                  </div>
                ) : null}
              </div>
            ) : (
          ...
          <Card className="space-y-3">
            ...
            <div className="space-y-2 text-sm">
              ...
              <div>
                Later actions affected: <span className="font-medium">{formatNumber(action.execution.laterActionsAffected)}</span>
              </div>
            </div>
          ...
          <Card className="space-y-3">
            ...
            <div className="space-y-2 text-sm">
              <div>
                Overlapping paths: <span className="font-medium">{formatNumber(action.execution.overlappingPaths.length)}</span>
              </div>
              {action.execution.overlappingPaths.length > 0 ? (
                ...
                <div className="text-sm text-[var(--ag-text-secondary)]">No overlapping paths were recorded for this action.</div>
              )}
            </div>
            {action.execution.snapshotId ? (
              <div className="text-sm text-[var(--ag-text-secondary)]">
                Recovery is linked to snapshot <span className="font-mono text-[var(--ag-text-primary)]">{action.execution.snapshotId}</span>.
              </div>
            ) : null}
          </Card>
