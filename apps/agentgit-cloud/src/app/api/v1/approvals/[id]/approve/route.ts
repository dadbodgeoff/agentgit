...
export async function POST(
  ...
  try {
    const result = await withWorkspaceAuthorityClient(workspaceSession.activeWorkspace.id, (client) =>
      client.resolveApproval(id, "approved", payload.data.comment),
    );
    return jsonWithRequestId(
    ...
    return toAuthorityRouteErrorResponse(error, "Could not submit the approval decision. Retry.", { requestId });
  }
}
